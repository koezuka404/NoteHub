...
const websocketAuthSubprotocol = "bearer"
...
func (c *WebSocketController) HandleDocument(e echo.Context) error {
	if c.production && e.Scheme() != "https" && e.Request().TLS == nil {
		return writeWebSocketHTTPError(
			e,
	...
	prepared, err := c.wsUseCase.PrepareConnection(
		ctx,
		usecase.PrepareWebSocketConnectionInput{
			UserID:     userID,
	...
	registered, err := c.wsUseCase.RegisterConnection(
		ctx,
		usecase.RegisterWebSocketConnectionInput{
			UserID:       userID,
			DocumentID:   documentID,
	...
	conn, err := wsUpgrader(c.checkOrigin).Upgrade(
		e.Response(),
		e.Request(),
		nil,
	)
	if err != nil {
		...
		_, _ = c.wsUseCase.UnregisterConnection(
			unregCtx,
			usecase.UnregisterWebSocketConnectionInput{
				UserID:       userID,
				DocumentID:   documentID,
				ConnectionID: connectionID,
			},
...
func (c *WebSocketController) HandleWorkspace(e echo.Context) error {
	...
	if err := c.wsUseCase.PrepareWorkspaceConnection(
		ctx,
		usecase.PrepareWorkspaceConnectionInput{
			UserID:      userID,
...
func (c *WebSocketController) authenticate(e echo.Context) (uuid.UUID, error) {
	rawToken := extractWebSocketToken(e.Request())

	if rawToken == "" {
		_ = writeWebSocketHTTPError(
			e,
	...
	claims, err := c.tokens.ValidateAccessToken(
		rawToken,
		c.now().UTC(),
	)
	if err != nil {
	...
	isRevoked, err := c.revoked.IsRevoked(
		ctx,
		claims.JTI,
	)
	if err != nil {
	...
	if isRevoked {
		_ = writeWebSocketHTTPError(
			...
			"ログイン状態が無効になりました再度ログインしてください",
		)

		return uuid.Nil, errResponseSent
	}

	user, found, err := c.users.FindByID(
		ctx,
		claims.UserID,
	)
	if err != nil {
...
func extractWebSocketToken(r *http.Request) string {
	...
	for _, value := range protocols {
		for _, protocol := range strings.Split(value, ",") {
			...
			if strings.EqualFold(protocol, websocketAuthSubprotocol) {
				continue
			}
...
func isLikelyAccessToken(value string) bool {
...
func (c *WebSocketController) sendInitialEvents(
	client *appws.Client,
	...
	c.sendEvent(
		client,
		appws.EventDocumentSync,
		appws.DocumentSyncData{
			DocumentID: prepared.DocumentID.String(),
	...
	c.sendEvent(
		client,
		appws.EventEditorsSync,
		appws.EditorsSyncData{
...
func (c *WebSocketController) handleClientMessage(
	client *appws.Client,
	raw []byte,
) {
	...
	case appws.EventDocumentEdit:
		...
		out, err := c.wsUseCase.ApplyDocumentEdit(
			ctx,
			usecase.ApplyDocumentEditInput{
		...
		payload, err := wsMarshalEvent(
			appws.EventDocumentUpdated,
			appws.DocumentUpdatedData{
...
func (c *WebSocketController) cleanupClient(
	client *appws.Client,
) {
	ctx, cancel := c.backgroundContext()

	out, err := c.wsUseCase.UnregisterConnection(
		ctx,
		usecase.UnregisterWebSocketConnectionInput{
			UserID:       client.UserID,
			DocumentID:   client.DocumentID,
	...
	if err != nil {
		log.Printf(
			"websocket: unregister connection failed document=%s user=%s connection=%s: %v",
			client.DocumentID,
			client.UserID,
			client.ConnectionID,
			err,
...
func (c *WebSocketController) sendEvent(
	client *appws.Client,
	eventType string,
	data any,
	now time.Time,
...
func (c *WebSocketController) sendError(
	client *appws.Client,
	code,
	message string,
) {
...
func (c *WebSocketController) sendUseCaseError(
	client *appws.Client,
	err error,
) {
	switch {
...
func toEditorEventData(
	editors []usecase.DocumentEditorInfo,
) []appws.EditorEventData {
	out := make(
		[]appws.EditorEventData,
		0,
...
func writeWebSocketHTTPError(
	e echo.Context,
	status int,
	code,
	message string,
) error {
	return appmiddleware.WriteError(
		e,
		status,
		code,
		message,
...
func handleWebSocketUseCaseError(
	e echo.Context,
	err error,
) error {
	switch {
	case errors.Is(
		err,
		usecase.ErrWebSocketConnectionLimitExceeded,
	):
		return writeWebSocketHTTPError(
			e,
			http.StatusConflict,
			"WEBSOCKET_CONNECTION_LIMIT_EXCEEDED",
			"WebSocket接続数の上限に達しています",
		)
...
func WebSocketControllerConfigFromApp(
	cfg *config.Config,
