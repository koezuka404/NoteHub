...
func Register(e *echo.Echo, deps Deps) {
	...
	if deps.WebSocket != nil {
		e.GET(
			...
			deps.WebSocket.HandleDocument,
		)

		e.GET(
			"/ws/workspaces/:workspaceId",
			deps.WebSocket.HandleWorkspace,
		)
	}
	...
	workspaces.PATCH(
		"/:workspaceId",
		deps.Workspace.Update,
		deps.RateLimit,
	)

	workspaces.DELETE(
		"/:workspaceId",
		deps.Workspace.Delete,
		deps.RateLimit,
	)
	...
	workspaces.POST(
		"/:workspaceId/members/:userId/suspend",
		deps.Account.Suspend,
		deps.RateLimit,
	)

	workspaces.POST(
		"/:workspaceId/members/:userId/reactivate",
		deps.Account.Reactivate,
		deps.RateLimit,
	)

	workspaces.POST(
		"/:workspaceId/members/:userId/delete-account",
		deps.Account.Delete,
		deps.RateLimit,
	)
