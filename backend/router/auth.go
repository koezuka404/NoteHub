...
func registerAuthRoutes(group *echo.Group, auth *controller.AuthController, authMiddleware, requireRefresh, originValidation, originOrCSRF, rateLimit echo.MiddlewareFunc) {
	...
	group.POST("/auth/logout", auth.Logout, authMiddleware, originOrCSRF, rateLimit)
