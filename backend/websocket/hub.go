...
func (h *Hub) DisconnectUserInWorkspace(workspaceID, userID uuid.UUID, payload []byte) {
	h.mu.Lock()
	targets := make([]*Client, 0)
	for documentID, clients := range h.documents {
		for client := range clients {
			if client.WorkspaceID != workspaceID || client.UserID != userID || !client.Ready.Load() {
				continue
			}
			targets = append(targets, client)
			delete(clients, client)
