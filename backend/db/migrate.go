...
func Migrate(gdb *gorm.DB) error {
	return gdb.Transaction(func(tx *gorm.DB) error {
		...
		statements := []string{
			...
			`CREATE UNIQUE INDEX IF NOT EXISTS uq_workspace_members_workspace_user ON workspace_members (workspace_id, user_id)`,
			...
			`CREATE UNIQUE INDEX IF NOT EXISTS uq_document_versions_revision ON document_versions (document_id, revision)`,
			...
			`CREATE INDEX IF NOT EXISTS idx_refresh_tokens_family_status ON refresh_tokens (family_id, status)`,
