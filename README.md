...
**CSRF / セッション:** `POST /api/auth/refresh` は **Origin 検証 + Refresh Cookie**、`POST /api/auth/logout` は **Origin 検証または Double Submit Cookie**（どちらか一方）で保護します。CSRF トークンは localStorage に保存せず、ログイン / リフレッシュ応答をメモリ上のみ保持します。
